...
* Controllers and apps have access to session (lazy loaded). Session tokens are very long and encrypted before sending
  as cookie
...
* Scope object ("self") for templates can be defined with TemplateScope.set_scope(class). Recommend using a sub-class
  of TemplateScope for maximum compatibility
